...
void setup() {
  ...
  letra = createFont("Sketch.ttf",200);
  ...
  limites.add(new Boundary(0, -5, width/2-615, 0)); //superior izq
...
void instrucciones(){
 background(0);
  ...
  text("mientras pulsas una vez la tecla R",300,250);
...
if(keyPressed){
  if(key == 'r'){
    ...
    if (i > 0) {
      esferas.add(new Esfera(random(width), -20, i/50, c));
    }
    }
  }
  }
